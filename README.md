...
多窗口协作通常由规划交给执行，再按任务需要进行复核；复核同时关注设计符合性与缺陷反例，实现问题回到执行方，方案问题回到规划方。角色与换岗边界见[常驻纪律](resident/tooluse-resident.md)；跨窗口材料的组织方式见 [project-doc-system 的「跨窗口交接」](skills/project-doc-system/SKILL.md#5-跨窗口交接一个文件每批清空)；高影响任务另按[高影响改动](#高影响改动)中的升档门处理。
...
配套包不依赖三件套或常驻纪律，不进入核心安装器的默认集合和版本记录。目录保持一个 Skill 一个文件夹。
