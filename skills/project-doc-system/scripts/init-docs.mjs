...
- 产品目标和成功标准优先核对并补充已有 README/规格，无合适载体才新建；来源按索引的目录内/目录外规则登记。
...
新增文档目录内的文档，在同一个提交里加进下表，路径相对本目录。目录外权威来源用表外普通 Markdown 链接注明范围与何时读，链接相对本索引；使用时核对可达性与内容，索引门禁不检查表外链接。
...
function main() {
  ...
  console.log("\n可机械探测的生长信号：");
  if (fired.length === 0) console.log("  未发现可机械探测的生长信号，不代表不需要补充文档。");
  else for (const line of fired) console.log(`  · ${line}`);
  console.log("  产品目标来源是否足够、关键流程能否定位，仍需人工确认。");
