...
**它没有下限豁免，但也没有常驻机制。** 三个交付 Skill 属于加码包，按影响开；这些规则的成本接近零，**内容上**任何一次改动都适用。它填的正是「低影响日常改动」那一格——那一格是刻意不设防的，代价就是下面这些事故。

> **「适用」不等于「会被加载」，这两件事在这里是分开的。** SKILL.md 正文只在这个 Skill 被选中之后才读得到，而它的 `description` 是故意收窄的：**只在用户点名、diff 已在扩散、或产物里已经出现残留时才命中**。也就是说它自动触发时偏事后救火；日常改动的**预防**位不靠它，靠把 tooluse README 的「低影响常驻块」复制进项目的 `CLAUDE.md` / `AGENTS.md`，或由人主动点名。一个 Skill 无法在自己的正文里让自己常驻。
